...
1. Collects every statement that matches the action: compiled YAML policies for the principal's roles, plus `PUBLIC` statements for anonymous callers.
...
|---|---|---|
| UserProgress | `user` | user_id |
| Favorite | `user` | user_id |
| UserAchievement | `user` | user_id |
