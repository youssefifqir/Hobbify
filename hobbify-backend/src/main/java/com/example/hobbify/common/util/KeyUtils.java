package com.example.hobbify.common.util;
...
public class KeyUtils {
    ...
    public static PrivateKey loadPrivateKey(final String pemPath) throws Exception {
        return parsePrivateKeyPem(readKeyFromResource(pemPath));
    }

    public static PublicKey loadPublicKey(final String pemPath) throws Exception {
        return parsePublicKeyPem(readKeyFromResource(pemPath));
    }

    /** Parses raw PEM text (not a path) — used when the key comes from an env var rather than a file. */
    public static PrivateKey parsePrivateKeyPem(final String pem) throws Exception {
        final String key = pem
    ...
    /** Parses raw PEM text (not a path) — used when the key comes from an env var rather than a file. */
    public static PublicKey parsePublicKeyPem(final String pem) throws Exception {
        final String key = pem
