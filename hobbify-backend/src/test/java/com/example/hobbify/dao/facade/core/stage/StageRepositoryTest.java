...
@DisplayName("StageDao Integration Tests")
class StageRepositoryTest {
    ...
    void setUp() {
        sample = new Stage();
        sample.setRef("stage-test-ref-001");
        sample.setTitle("test-title");
        sample.setOrder(java.math.BigDecimal.ONE);
    }
    ...
    class Save {
        ...
        void persistsEntity() {
            Stage saved = dao.save(sample);

            assertThat(saved.getId()).isNotNull().isPositive();
            assertThat(saved.getRef()).isEqualTo("stage-test-ref-001");
        }
    ...
    class FindById {
        ...
        void returnsSavedEntity() {
            ...
            Optional<Stage> found = dao.findById(saved.getId());
        ...
        void returnsEmpty_whenNotFound() {
            Optional<Stage> found = dao.findById(-999L);
    ...
    class FindByRef {
        ...
        void returnsEntityByRef() {
            ...
            Stage found = dao.findByRef("stage-test-ref-001");

            assertThat(found).isNotNull();
            assertThat(found.getRef()).isEqualTo("stage-test-ref-001");
        }

        @Test
        @DisplayName("returns null for unknown ref")
        void returnsNull_whenRefNotFound() {
            Stage found = dao.findByRef("non-existent-ref");
