...
class AchievementRepositoryTest {
    ...
    void setUp() {
        ...
        sample.setRef("achievement-test-ref-001");
        sample.setName("test-name");
        sample.setDescription("test-description");
        sample.setThreshold(java.math.BigDecimal.ONE);
    }
    ...
    class Save {
        ...
        void persistsEntity() {
            Achievement saved = dao.save(sample);

            assertThat(saved.getId()).isNotNull().isPositive();
            assertThat(saved.getRef()).isEqualTo("achievement-test-ref-001");
        }
    ...
    class FindById {
        ...
        void returnsSavedEntity() {
            ...
            Optional<Achievement> found = dao.findById(saved.getId());
        ...
        void returnsEmpty_whenNotFound() {
            Optional<Achievement> found = dao.findById(-999L);
    ...
    class FindByRef {
        ...
        void returnsEntityByRef() {
            ...
            Achievement found = dao.findByRef("achievement-test-ref-001");

            assertThat(found).isNotNull();
            assertThat(found.getRef()).isEqualTo("achievement-test-ref-001");
        }

        @Test
        @DisplayName("returns null for unknown ref")
        void returnsNull_whenRefNotFound() {
            Achievement found = dao.findByRef("non-existent-ref");
