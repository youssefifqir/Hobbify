...
@DisplayName("UserProgressService Unit Tests")
class UserProgressServiceTest {
    ...
    private UserProgress sample;

    @BeforeEach
    void setUp() {
        sample = new UserProgress();
        sample.setId(1L);
        sample.setRef("userprogress-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<UserProgress> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<UserProgress> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<UserProgress> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            ...
            when(dao.save(any(UserProgress.class))).thenReturn(sample);

            UserProgress result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(UserProgress.class));
        }
        ...
        void generatesRef_whenMissing() {
            UserProgress toSave = new UserProgress();
            when(dao.save(any(UserProgress.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            UserProgress result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            ...
            UserProgress result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("userprogress-ref-001")).thenReturn(sample);

            UserProgress result = service.findByRef("userprogress-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("userprogress-ref-001");
            verify(dao).findByRef("userprogress-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            UserProgress result = service.findByRef("  ");
