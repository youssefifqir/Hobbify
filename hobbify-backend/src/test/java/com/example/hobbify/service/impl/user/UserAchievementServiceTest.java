...
@DisplayName("UserAchievementService Unit Tests")
class UserAchievementServiceTest {

    @Mock
    private UserAchievementDao dao;

    @InjectMocks
    private UserAchievementServiceImpl service;

    private UserAchievement sample;

    @BeforeEach
    void setUp() {
        sample = new UserAchievement();
        sample.setId(1L);
        sample.setRef("userachievement-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<UserAchievement> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<UserAchievement> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<UserAchievement> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            UserAchievement toSave = new UserAchievement();
            when(dao.save(any(UserAchievement.class))).thenReturn(sample);

            UserAchievement result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(UserAchievement.class));
        }
        ...
        void generatesRef_whenMissing() {
            UserAchievement toSave = new UserAchievement();
            when(dao.save(any(UserAchievement.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            UserAchievement result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            UserAchievement toCreate = new UserAchievement();
            when(dao.save(any(UserAchievement.class))).thenReturn(sample);

            UserAchievement result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("userachievement-ref-001")).thenReturn(sample);

            UserAchievement result = service.findByRef("userachievement-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("userachievement-ref-001");
            verify(dao).findByRef("userachievement-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            UserAchievement result = service.findByRef("  ");
