...
class StageServiceTest {
    ...
    private Stage sample;

    @BeforeEach
    void setUp() {
        sample = new Stage();
        sample.setId(1L);
        sample.setRef("stage-ref-001");
    }
    ...
    class FindAll {
        ...
        void returnsEntities() {
            ...
            List<Stage> result = service.findAll();
    ...
    class FindById {
        ...
        void returnsEntity_whenFound() {
            ...
            Optional<Stage> result = service.findById(1L);
        ...
        void returnsNull_whenNotFound() {
            ...
            Optional<Stage> result = service.findById(99L);
    ...
    class Save {
        ...
        void persistsEntity() {
            Stage toSave = new Stage();
            when(dao.save(any(Stage.class))).thenReturn(sample);

            Stage result = service.save(toSave);

            assertThat(result).isNotNull();
            assertThat(result.getId()).isEqualTo(1L);
            verify(dao).save(any(Stage.class));
        }
        ...
        void generatesRef_whenMissing() {
            Stage toSave = new Stage();
            when(dao.save(any(Stage.class))).thenAnswer(inv -> inv.getArgument(0));
        ...
        void returnsNull_whenEntityIsNull() {
            Stage result = service.save(null);
    ...
    class Create {
        ...
        void delegatesToSave() {
            Stage toCreate = new Stage();
            when(dao.save(any(Stage.class))).thenReturn(sample);

            Stage result = service.create(toCreate);
    ...
    class FindByRef {
        ...
        void returnsEntity_whenRefMatches() {
            when(dao.findByRef("stage-ref-001")).thenReturn(sample);

            Stage result = service.findByRef("stage-ref-001");

            assertThat(result).isNotNull();
            assertThat(result.getRef()).isEqualTo("stage-ref-001");
            verify(dao).findByRef("stage-ref-001");
        }

        @Test
        @DisplayName("returns null for blank ref without hitting the DAO")
        void returnsNull_whenRefIsBlank() {
            Stage result = service.findByRef("  ");
